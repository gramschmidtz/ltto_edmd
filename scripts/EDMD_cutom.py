...
def main():
    ...
    PsiX1 = custom_observ_1(dimension, X1)
    PsiX2 = custom_observ_1(dimension, X2)
    ...
    Psi = [custom_observ_1(dimension, X[0])]
    U_hist = []
    t_days = [0.0]
    tau_hist = [0.0]
    ...
    while t_days[-1] < T_END_DAYS and step < 100_000:
        ...
        lam = psi_next[idx_lam].item()
        eta = psi_next[idx_eta].item()
        kap = psi_next[idx_kap].item()
    ...
    fig.suptitle(f"EDMD + Sundman transform, the number of observer fuction = {m}")
...
def taus():
    ...
    PsiX[:,0:1] = custom_observ_1(dimension, X[:,0:1])
